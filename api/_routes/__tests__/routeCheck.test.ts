...
const generateRouteForecastMock = vi.fn()
...
vi.mock('../../_lib/routeForecast.js', () => ({
  generateRouteForecast: generateRouteForecastMock,
}))
...
vi.mock('../../_lib/geocode.js', () => ({
  ...
  mockGeocode: vi.fn((input: string) => {
    const known: Record<string, { lat: number; lng: number }> = {
      ...
      'tan binh': { lat: 10.8015, lng: 106.6526 },
      'binh thanh': { lat: 10.8032, lng: 106.7078 },
...
function makeForecastSummary(overrides = {}) {
  return {
    source: 'rule-based',
    riskLevel: 'low',
    confidence: 'medium',
    summary: 'Route risk is currently low.',
    peakWindow: 'Within the next 1-3 hours',
    reasons: ['No confirmed or forecast flood exposure is currently detected on this route.'],
    dataPoints: {
      confirmedFloodsOnRoute: 0,
      forecastFloodsOnRoute: 0,
      rainfallNext6hMm: null,
      alternativeFloodReduction: 0,
    },
    ...overrides,
  }
}
...
describe('POST /route-check - mock mode', () => {
  ...
  beforeAll(async () => {
    ...
    storeWithFlood = {
      base: [makeFlood(10.769, 106.6874)],
      simulated: [],
    }
  ...
  beforeEach(() => {
    ...
    generateRouteForecastMock.mockResolvedValue(makeForecastSummary())
  })
  ...
  it('null origin -> 400', async () => {
    const res = await request(app).post('/').send({ origin: null, destination: 'tan binh' })
    expect(res.status).toBe(400)
  })
  ...
  it('unknown address -> 400', async () => {
    const res = await request(app).post('/').send({ origin: 'xyzzy-unknown', destination: 'tan binh' })
    expect(res.status).toBe(400)
  })

  it('valid string addresses -> 200 with route coords', async () => {
    const res = await request(app).post('/').send({ origin: 'q7', destination: 'tan binh' })
    expect(res.status).toBe(200)
    expect(res.body.success).toBe(true)
    expect(res.body.route.coords.length).toBeGreaterThan(1)
    expect(res.body.forecast.summary).toBe('Route risk is currently low.')
  })
  ...
  it('flood on route -> non-empty floodZones and English alertText', async () => {
    process.env.DATA_MODE = 'mock'
    const mod = await import('../../_routes/routeCheck.js')
    const floodApp = express()
    floodApp.use(express.json())
    floodApp.use('/', mod.default(storeWithFlood))

    const res = await request(floodApp).post('/').send({ origin: 'q7', destination: 'tan binh' })
    expect(res.status).toBe(200)
    expect(res.body.floodZones.length).toBeGreaterThan(0)
    expect(res.body.alertText).toMatch(/Warning:/)
  })

  it('uses directions geometry when available', async () => {
...
describe('POST /route-check - live mode', () => {
  ...
  it('calls get_floods_in_bbox RPC and returns 200', async () => {
    ...
    const res = await request(app).post('/').send({ origin: 'hcmc', destination: 'binh thanh' })
    expect(res.status).toBe(200)
    expect(getSb().rpc).toHaveBeenCalledWith(
      'get_floods_in_bbox',
  ...
  it('passes recent route history context into forecast generation when anonymous history exists', async () => {
    getSb().rpc.mockResolvedValue({ data: [], error: null })
      ...
      data: [
        {
          ...
          origin_label: 'hcmc',
          destination_label: 'binh thanh',
          risk_level: 'high',
          confirmed_flood_count: 2,
    ...
    const res = await request(app)
      .post('/')
      .set('x-anonymous-id', 'anon-route-history')
      .send({ origin: 'hcmc', destination: 'binh thanh' })

    expect(res.status).toBe(200)
    expect(generateRouteForecastMock).toHaveBeenCalledWith(
      expect.objectContaining({
        recentHistory: expect.objectContaining({
          recentChecks: 1,
          sameCorridorChecks: 1,
          sameCorridorHighRiskCount: 1,
        }),
      }),
    )
  })
...
describe('POST /route-check - forecast warnings', () => {
  ...
  function makeForecastFlood(district: string) {
    return {
      id: 'forecast-1',
      street_name: `Area ${district}`,
      district,
      city: 'Ho Chi Minh City',
      coordinates: { lat: 10.79, lng: 106.7044 },
      depth_cm: null,
      ...
      is_simulated: false,
      is_forecast: true,
      forecast_valid_until: new Date(Date.now() + 3 * 3_600_000).toISOString(),
    }
  }
  ...
  beforeEach(() => {
    ...
    generateRouteForecastMock.mockResolvedValue(
      makeForecastSummary({
        riskLevel: 'medium',
        summary: 'Route risk is elevated. Conditions should be rechecked before departure.',
        peakWindow: 'Within the next 3 hours',
        reasons: ['1 forecast flood zone may affect the route soon.'],
        dataPoints: {
          confirmedFloodsOnRoute: 0,
          forecastFloodsOnRoute: 1,
          rainfallNext6hMm: null,
          alternativeFloodReduction: 0,
        },
      }),
  ...
  const DESTINATION = { lat: 10.8032, lng: 106.7078 }

  it('forecast flood in bbox -> forecast warning in warnings[] but not in floodZones', async () => {
    const forecastFlood = makeForecastFlood('Binh Thanh')
    ...
    const hasWarning = res.body.warnings.some((warning: string) => warning.includes('Expected flood risk'))
    expect(hasWarning).toBe(true)
    expect(res.body.forecast.riskLevel).toBe('medium')
  })

  it('forecast-only risk can still return an alternativeRoute when a safer detour exists', async () => {
    const forecastFlood = makeForecastFlood('Binh Thanh')
    getSb().rpc.mockResolvedValue({ data: [forecastFlood], error: null })
    getRouteAlternativesMock.mockResolvedValue([
      [
        { lat: 10.7769, lng: 106.7009 },
        { lat: 10.79, lng: 106.7044 },
        { lat: 10.8032, lng: 106.7078 },
      ],
      [
        { lat: 10.7769, lng: 106.7009 },
        { lat: 10.782, lng: 106.695 },
        { lat: 10.8032, lng: 106.7078 },
      ],
    ])
    ...
    expect(res.status).toBe(200)
    expect(res.body.alternativeRoute).not.toBeNull()
    expect(res.body.alternativeRoute.floodZones).toHaveLength(0)
    expect(res.body.warnings.some((warning: string) => warning.includes('Safe alternative route available'))).toBe(true)
  })
  ...
  it('forecast warning includes rainfall mm when weather available', async () => {
    ...
    expect(res.status).toBe(200)
    const forecastWarning = res.body.warnings.find((warning: string) => warning.includes('Expected flood risk'))
  ...
  it('forecast warning falls back to generic string when weather unavailable', async () => {
    ...
    const forecastWarning = res.body.warnings.find((warning: string) => warning.includes('Expected flood risk'))
    expect(forecastWarning).toMatch(/weather forecast conditions/)
